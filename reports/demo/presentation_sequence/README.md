...
| `demo_navigation_oracle_counterfactual_rollout_05s.mp4` | 25-second 1080p 5-second-horizon real ViZDoom counterfactual rollout. CV, PointNav oracle, A* oracle, GT, and ours are restarted from the same sample pose and followed as separate simulator branches, so the first-person videos diverge instead of reusing the same recorded RGB. |
