"""Run the 3s spatial relation retraining ablation end to end."""
...
DEFAULT_CONFIGS = [
    Path("configs/ablations/train_spatial_topk_graph_03s.yaml"),
    Path("configs/ablations/train_spatial_no_graph_03s.yaml"),
    Path("configs/ablations/train_spatial_full_attention_03s.yaml"),
    Path("configs/ablations/train_spatial_local_grid_03s.yaml"),
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run 3s spatial relation ablation training and summarization.")
...
def main() -> None:
    ...
    run([args.python, "scripts/summarize_spatial_relation_ablation.py"])
