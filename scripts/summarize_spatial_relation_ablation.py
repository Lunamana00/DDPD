"""Summarize 3s spatial relation retraining ablation runs."""
...
VARIANTS = [
    ...
    ("no_graph", "No spatial relation module"),
    ("full_attention", "Full token attention, self excluded"),
    ("local_grid", "Fixed 8-neighbor grid attention"),
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize spatial relation ablation metrics.")
    parser.add_argument("--runs-root", type=Path, default=Path("runs/spatial_relation_ablation_v4_03s"))
    parser.add_argument("--output-json", type=Path, default=Path("outputs/spatial_relation_ablation_v4_03s/results.json"))
    parser.add_argument("--output-md", type=Path, default=Path("reports/spatial_relation_ablation_v4_03s.md"))
...
def summarize_variant(runs_root: Path, name: str, description: str) -> dict[str, Any]:
    ...
    row.update(
        {
            "spatial_relation_type": config.get("spatial_relation_type", name),
...
def write_markdown(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        lines.append(
            "| {variant} | {available} | {best_epoch} | {test_ADE} | {test_FDE} | "
    ...
    if available:
        ...
        lines.extend(["", "- No completed variant metrics found yet."])
