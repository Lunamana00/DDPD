...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--split", choices=["episode", "map"], default="episode")
...
def make_episode_splits(samples: list[dict[str, Any]], seed: int) -> dict[str, list[str]]:
    episode_ids = sorted({sample["episode_id"] for sample in samples})
    rng = random.Random(seed)
    rng.shuffle(episode_ids)
    if len(episode_ids) == 1:
        groups = {"train": episode_ids, "val": episode_ids, "test": episode_ids}
    else:
        n_train = max(1, int(round(len(episode_ids) * 0.7)))
        n_val = max(1, int(round(len(episode_ids) * 0.15))) if len(episode_ids) >= 3 else 1
        train_eps = episode_ids[:n_train]
        val_eps = episode_ids[n_train : n_train + n_val]
        test_eps = episode_ids[n_train + n_val :]
        if not val_eps:
            val_eps = train_eps[-1:]
        if not test_eps:
            test_eps = val_eps
        groups = {"train": train_eps, "val": val_eps, "test": test_eps}

    splits = {name: [] for name in groups}
    for sample in samples:
        for split_name, split_episodes in groups.items():
            if sample["episode_id"] in split_episodes:
...
def build_samples(
    ...
    for episode in manifest["episodes"]:
        ...
        for center_idx in range(first_center, last_center + 1, max(1, stride)):
            ...
            sample_id = f"{episode_id}_t{center_record['step']:06d}"
            sample = {
                "sample_id": sample_id,
                "episode_id": episode_id,
                "center_step": center_record["step"],
                "rgb_history_paths": [item["frame_path"] for item in history_records],
                ...
                "depth_history_paths": [item.get("depth_path") for item in history_records],
                "labels_history_paths": [item.get("labels_path") for item in history_records],
                "metadata": {
                    "map_id": manifest.get("map"),
    ...
    splits = make_episode_splits(samples, seed)
    ...
    dataset_manifest = {
        ...
        "history_frames": history_frames,
        "future_steps": future_steps,
        "split_strategy": "episode-disjoint",
